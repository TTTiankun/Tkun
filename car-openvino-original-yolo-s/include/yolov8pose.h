...
public:
    ...
    bool process(cv::Mat &source,std::vector<Object> &detected_armors,std::vector<Object> &detected_robots);//总过程
    bool pre_img(cv::Mat &frame);//图像预处理
    cv::Mat letterbox(cv::Mat &source);//图像转化器
    bool inference();//推理
    void extract();//结果提取
    void find_result(std::vector<Object> &detected_armors,
                std::vector<Object> &detected_robots);//储存结果
    
    //展示函数
    void show_result(cv::Mat &source,std::vector<Object> &detected_armors,std::vector<Object> &detected_robots);

private:

    //推理参数区
    int GPU_mode;//是否使用GPU
    ...
    cv::Point class_id;//类别id
    double maxclassScore;//最大类别置信度
    float maxscore;//最大置信度(推理分类用)
    int model_input_w;//模型输入宽
    int model_input_h;//模型输入高
    int model_output_w;//模型输出宽
    int model_output_h;//模型输出高
