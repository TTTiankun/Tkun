...
std::string model_number_path = "/home/jason/Desktop/test-speed/openvino/yolov8-cls-int.onnx";
...
int main(){
    ...
    while(1){
        ...
        if(detected_armors.size() == 0){
            ...
            number.asyncinference(detected_armors);
            auto end_number = std::chrono::high_resolution_clock::now();
            auto duration_number = std::chrono::duration_cast<std::chrono::milliseconds>(end_number - start_number);
            std::cout << "Number inference time: " << duration_number.count() << "ms" << std::endl;
        }
