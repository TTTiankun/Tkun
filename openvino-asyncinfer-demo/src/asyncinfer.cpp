...
AsyncInfer::AsyncInfer(){
    std::cout<<"AsyncInference begin!"<<std::endl;
}

AsyncInfer::~AsyncInfer(){}

bool AsyncInfer::init(int GPU_mode, int async_mode, std::string model_path,
    ...
    */
    ...
    if(async_mode == 1){
        ...
        std::cout<<"The syncinference init success!"<<std::endl;
    }
    cv::waitKey(1000);
    return true;
}

bool AsyncInfer::pre_img(cv::Mat &frame, ov::Tensor &input_tensor){
    ...
    cv::threshold(data_mat,data_mat,0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);//二值化
    return true;
}

bool AsyncInfer::syncinference(cv::Mat &frame){
    
    pre_img(frame, curr_input_tensor);
    curr_request.infer();//开始推理
    
    this->curr_output_tensor = this->curr_request.get_output_tensor();
...
bool AsyncInfer::asyncinference(cv::Mat &next_frame){

    int condtion = 0;//判断是否有下一帧图片
    
    if(next_frame.empty()){
        std::cout<<"The next_frame is empty!"<<std::endl;
        condtion = 0;
    }else{
        pre_img(next_frame, next_input_tensor);
        next_request.start_async();//开始异步推理
        curr_request.wait();//等待当前推理完成
