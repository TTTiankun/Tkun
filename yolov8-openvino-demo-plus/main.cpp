...
std::string model_best_path = "/home/jason/Desktop/test-speed/openvino/best.onnx";
...
std::vector<cv::Scalar> colors_best = { cv::Scalar(255, 0, 0), cv::Scalar(255, 0, 255),cv::Scalar(0, 0, 255),cv::Scalar(0, 255, 255)};

std::vector<std::string> class_names_best = {"armor","car"};
...
void Yolov8_pose(){
    cap.open(video_path2);
    
    class Yolov8pose yolo_pose;
    yolo_pose.init(GPU_model, model_best_path, score_threshold, 
                NMS_threshold, colors_best, class_names_best);
...
void Yolov8(){
    ...
    yolo.init(GPU_model, model_path, score_threshold, 
                NMS_threshold, colors_best, class_names);
