...
#include <inference_engine.hpp>//额外添加的头文件
...
bool Numberdetector::syncinference(cv::Mat &frame){
    
    pre_img(frame, this->curr_input_tensor);

    curr_request.infer();//开始推理

    curr_output_tensor = curr_request.get_output_tensor();
    ...
    //测试代码
    // while(InferenceEngine::StatusCode::RESULT_NOT_READY){
    //     std::cout<<"Busy!!!Busy!!!Busy!!!"<<std::endl;
    // }
